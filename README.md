...
- **🚧 Assessment v2 (in progress):** adaptive diagnostic over a curated **Question Bank** (`mentoros/assessment/`, `data/assessment/grammar.json` — 110 items, all 21 grammar topics) — selector + stop-by-confidence, server-side grading, event-sourced session (`POST /assessment/start` · `/assessment/answer`). Feeds the Knowledge Projection; with enough evidence the CEFR projection locks. Next: a CAT-style "narrowing" selector so one run hones in on the level. Produces higher-quality events; the core is untouched.
