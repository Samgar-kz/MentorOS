...
- **🚧 Assessment v2 (in progress):** adaptive diagnostic over a curated **Question Bank** (`mentoros/assessment/`, `data/assessment/grammar.json` — 110 items, all 21 grammar topics) — **narrowing selector** (estimate ability θ on the CEFR scale, ask near θ, skip confident topics → one run hones in on the level instead of spreading 1/topic), stop-by-confidence, server-side grading, event-sourced session (`POST /assessment/start` · `/assessment/answer`). On finish, levels below the result are marked known so the CEFR projection + plan reflect it. Produces higher-quality events; the core is untouched.
